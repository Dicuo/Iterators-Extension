(function(Scratch) {
    ...
    if (!Scratch.extensions.unsandboxed) {
        throw new Error('\'Iterators (old)\' must run unsandboxed!');
    }
    ...
    class IteratorTypeOld {
        customId = "divIteratorOld"
        consumed = 0
        done = false
        ...
        getIterChain() {
            ...
            .map(k => typeof k === "string" ? k
                : k.kind && k.args ? `${k.kind}(${
                    k.args.map(i => i instanceof IteratorTypeOld ? i.getIterChain() : i.toString()).join(",")
                })`
                : ""
            )
            .join("↦")
        ...
        chainIter(kind, gen) {return new IteratorTypeOld(this.kind.concat(kind), gen)}
        ...
        static toIterator(x) {
            if(x instanceof IteratorTypeOld) return x;
            return new IteratorTypeOld();
        }
    ...
    const divIteratorOld = {
        Type: IteratorTypeOld,
        Block: {
            blockType: BlockType.REPORTER,
            //blockShape: "divIteratorOld-iter", 
            blockShape: vm.pmVersion ? BlockShape.ARROW : "divIteratorOld-iter",
            forceOutputType: "IteratorOld",
            allowDropAnywhere: true,
            disableMonitor: true
        },
        Argument: {
            //blockShape: "divIteratorOld-iter", 
            shape: vm.pmVersion ? BlockShape.ARROW : "divIteratorOld-iter",
    ...
    function registerString() {
        ...
        vm.runtime.divIterables.set("string", {
            ...
            toIter() {
                const str = this;
                return new vm.divIteratorOld.Type("String", function*() {
    ...
    function registerjwArray() {
        ...
        vm.runtime.divIterables.set("array", {
            ...
            toIter() {
                const {array} = this;
                return new vm.divIteratorOld.Type("Array", function*() {
    ...
    function registerdogeObject() {
        ...
        vm.runtime.divIterables.set("object", {
            ...
            toIter() {
                const {object} = this;
                return new vm.divIteratorOld.Type("Object", function*() {
    ...
    class Extension {
        constructor() {
            vm.divIteratorOld = divIteratorOld
            vm.runtime.registerSerializer("divIteratorOld",
                _ => null,
                _ => new IteratorTypeOld()
            )
            vm.runtime.registerCompiledExtensionBlocks('divIteratorOld', this.getCompileInfo());
        ...
        getInfo = () => ({
            ...
            blocks: [
                ...
                '---',
                {
                    ...
                    arguments: {
                        iter1: divIteratorOld.Argument,
                        iter2: divIteratorOld.Argument,
                    },
                    ...divIteratorOld.Block
                },
                {
        ...
        getCompileInfo = () => ({
            ...
            js: {
                iterAdvance(node, compiler, imports) {
                    compiler.source += /*js*/`yield* vm.divIteratorOld.Type.toIterator(${compiler.descendInput(node.iter).asUnknown()}).yieldNext(thread);\n`
                },
                iterNext(node, compiler, imports) {
                    return new imports.TypedInput(
                 /*js*/`((yield* vm.divIteratorOld.Type.toIterator(${compiler.descendInput(node.iter).asUnknown()}).yieldNext(thread)) ?? '')`
                ...
                iterCollectToType(node, compiler, imports) {
                    ...
                    return new imports.TypedInput(
                /*js*/ `(yield* vm.divIteratorOld.Type.toIterator(${compiler.descendInput(node.iter).asUnknown()}).iterTo('${node.type}', function*(){${yielder}}, thread))`
                    , imports.TYPE_UNKNOWN)
                },
                ...
                iterAdapterMap(node, compiler, imports) {
                    ...
                    return new imports.TypedInput(
                 /*js*/`(yield* (function*() {\n`
                      +`    const ${iter} = vm.divIteratorOld.Type.toIterator(${compiler.descendInput(node.iter).asUnknown()});\n`
                ...
                iterAdapterInspect: (node, compiler, imports) => {
                    ...
                    const src = compiler.source
                    compiler.source = ""
                    compiler.descendStack(node.substack, new imports.Frame(true, "divIteratorOld.iterAdapterInspect"))
                    compiler.yieldLoop()
                ...
                iterTermForEach: (node, compiler, imports) => {
                    ...
                    const src = compiler.source
                    compiler.source = ""
                    compiler.descendStack(node.substack, new imports.Frame(true, "divIteratorOld.iterTermForEach"))
                    compiler.yieldLoop()
                    const substack = compiler.source
                    compiler.source = src + 
                 /*js*/`const ${iter} = vm.divIteratorOld.Type.toIterator(${compiler.descendInput(node.iter).asUnknown()});\n`
        ...
        iterDone({iter}) {
            iter = IteratorTypeOld.toIterator(iter)
            return iter.done
        }

        // Iterables
        iterOver({itrbl}) {
            if(itrbl instanceof IteratorTypeOld) return itrbl;
            //if(itrbl[divIteratorOld.Interface.toIter]) return itrbl[divIteratorOld.Interface.toIter]()
            return [...vm.runtime.divIterables.values()].find(({type}) => type == itrbl.constructor)?.toIter?.apply(itrbl) ?? new IteratorTypeOld()
        }
        ...
        iterRange({start, end}) {
            return new IteratorTypeOld({kind: "Range", args: [start, end]}, function*() {
        ...
        iterAdapterChain({iter1, iter2}) {
            iter1 = IteratorTypeOld.toIterator(iter1)
            iter2 = IteratorTypeOld.toIterator(iter2)
            ...
            return iter.chainIter({kind: "Flatten", args: [depth]}, function*() {
                const flat = function*(iter, depth) {
                    if(depth === 0) yield* iter;
                        ...
                        const item = yield* iter.yieldNext(this.thread)
                        if(iter.done) return;
                        if(item instanceof IteratorTypeOld) yield* flat(item, depth-1);
        ...
        //     return new IteratorTypeOld("Array", function*() {
        //         for(let i = 0; i < array.length; i++) yield array[i]
        //     })
        // }
